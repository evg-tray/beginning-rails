class Search #< ApplicationRecord
  ...
  def get_result
    query = "SELECT 'trains'.*, rsr_start.departure, rsr_end.arrival FROM 'trains'
      ...
      WHERE rsr_start.railway_station_id = #{start_station_id} AND
      rsr_end.railway_station_id = #{end_station_id} AND
      'trains'.route_id IN (
        ...
        WHERE order_start.railway_station_id = #{start_station_id}
        AND order_end.railway_station_id = #{end_station_id}
